...
<form action="/admin/notice" method="post">
	...
	<div class="notice margin-top">
		...
		<button type="button" onclick="location.href='/admin/reg-notice'" style="float:right;">공지사항 등록</button>
		<table class="table">
			<thead>
				<tr>
			...
			<c:forEach var="n" items="${noticeList}">
				<tr>
					...
					<td>${n.num}</td>
					<td class="title indent text-align-left"><a href="/admin/notice/${n.noticeID}">${n.title}</a></td>
					<td>
						<c:choose>
...
<div style="display: block; text-align: center;">		
	<c:if test="${page.startPage != 1 }">
		<a href="/admin/notices?nowPage=${page.startPage - 1 }&cntPerPage=${page.cntPerPage}&condition=${page.condition}&keyword=${page.keyword}">이전</a>
	</c:if>
	<c:forEach begin="${page.startPage }" end="${page.endPage }" var="p">
		<c:choose>
			...
			<c:when test="${p != page.nowPage }">
				<a href="/admin/notices?nowPage=${p }&cntPerPage=${page.cntPerPage}&condition=${page.condition}&keyword=${page.keyword}">${p }</a>
			</c:when>
		</c:choose>
	</c:forEach>
	<c:if test="${page.endPage != page.lastPage}">
		<a href="/admin/notices?nowPage=${page.endPage+1 }&cntPerPage=${page.cntPerPage}&condition=${page.condition}&keyword=${page.keyword}">다음</a>
	</c:if>
</div>
<!-- 공지사항 검색 기능 -->
